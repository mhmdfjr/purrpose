...
import { generateRuleBasedSuggestion } from "../services/reportSuggestion";
...
function getWeekRange(weekId: string): { startDate: string; endDate: string } {
...
function getPreviousWeekId(now: Date): string {
 ...
 */
export const weeklyCycleJob = onSchedule("0 0 * * 1", async () => {
  const now = new Date();
  ...
  for (const userDoc of usersSnap.docs) {
    ...
    try {
      ...
      const ruleBasedSuggestion = generateRuleBasedSuggestion({
        humbleScore,
        hustleScore,
        totalScore,
        balanceIndex,
      ...
      if (processed % 100 === 0) logger.info(`[weeklyCycle] processed ${processed}/${usersSnap.size}`);
    } catch (e) {
...
// Export helpers for testing / callable manual trigger
export { getISOWeekId, getWeekRange, getPreviousWeekId };
